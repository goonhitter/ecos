...
local minigameEnabled = false

local enableMinigameCompletion = tab5.new('switch', {
	text = 'Enable Auto-Minigame'
}) enableMinigameCompletion.set(false)
enableMinigameCompletion.event:Connect(function(bool)
	minigameEnabled = bool
end)

local function spawnSpaceText()
...
game:GetService('RunService').RenderStepped:Connect(function()
	if game.Players.LocalPlayer.PlayerGui:FindFirstChild('Stats') then
		if game.Players.LocalPlayer.PlayerGui.Stats:FindFirstChild('Frame') then
			if minigameEnabled and game.Players.LocalPlayer.PlayerGui.Stats.Frame.Minigame.Visible == true and spawned == false then
				game.ReplicatedStorage.Events.UpdateScore:FireServer(0)
				local Camera = game.Workspace.CurrentCamera
				local Viewport = Camera.ViewportSize
				local Position = Vector2.new(Viewport.X / 2, Viewport.Y / 2)

				circle = Drawing.new("Text")
				circle.Text = 'Press SPACE'
				...
				circle.Outline = 2
				circle.Size = 100
				circle.OutlineColor = Color3.new(0, 0, 0)

				spawned = true
			elseif game.Players.LocalPlayer.PlayerGui.Stats.Frame.Minigame.Visible == false then
				if circle ~= nil then
					circle:Destroy()
					circle = nil
				end
...
uis.InputBegan:Connect(function(key)
	if minigameEnabled == true and key.KeyCode == Enum.KeyCode.Space and spawned == true then
		game.ReplicatedStorage.Events.UpdateScore:FireServer(0)
